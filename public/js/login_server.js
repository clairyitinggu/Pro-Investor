...
app.post("/login", function (request, response) {
    ...
    connection.query(
        ...
        function (error, results) {
            if (results.length > 0) {
                // matched
                connection.query('select InvestmentName from userInvestment where userEmail = "' + username + '"',
                    function (error, result) {
                        ...
                        listener.on("connection", function (socket) {
                            socket.on("message", function (msg) {
                                ...
                                if (msg.startsWith("delete")) { //got delete query
                                    connection.query(msg, function (error, result) {
                                        console.log("data deleted from db");
                                    });
                                }
                                if (msg.startsWith("select")) {  //got select message
                                    connection.query(msg, function (error, result) {
                                        if (result != null && result.length == 0) {
                                            var foo = "InvestmentName = ";
                                            var stockToBeAdded = msg.slice(msg.indexOf(foo) + foo.length);
                                            var insertQuery = 'insert into userInvestment (userEmail,InvestmentName,InvestmentHolding,InvestmentTime) values ("' + username + '",UPPER(' + stockToBeAdded + '),0,now());';
                                ...
                                if (msg.startsWith("insert")) {  //got select message
                                    connection.query(msg, function (error, result) {
                                        if (error != null)
                                            console.log("stock added to db");
                                    });
